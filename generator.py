...
def train_generator_queue(out_channels, classes, size, batch_size=None, dump_mem=False, every_flip=False, workers=1):
    ...
    def prep_out(o):
        if len(o.shape) == 2:
            result = o.reshape((o.shape[0], o.shape[1], 1))
            alpha = 1.0 - result
    ...
    def generate_loop(dump_mem):
        try:
            ...
            inputs = load_classes('dataset/', class_tuple[0])
            outputs = list(map(prep_out, load_classes('dataset/', class_tuple[1])))
            random.seed(1)
            area = size[0] * size[1]
            while not shutdown:
                results = []
